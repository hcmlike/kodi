...
# Standard libs
import urllib,urllib2,unicodedata,string,re,os,json
...
def getPageContent(url, isJson=False):
...
def buildMedia4Eps(dirnameURL, ajaxEpsURL, thumbnail):
  ...
  mode = 1
  content = getPageContent(ajaxEpsURL, isJson=True)
  for item in content:
...
def homeScreen(url):
  ...
  if len(filmTypeList) > 0:
    ...
    # TODO: Tạm thời ngưng chức năng này vì có một số lỗi cần được fix.
...
def searchFilmOnYoutube(url):
  ...
  try:
    ...
    keyb.doModal()
    if (keyb.isConfirmed()):
      inputText  = keyb.getText()
      searchText = urllib.quote_plus(inputText)
      writeTempfile(inputText)
  except: pass
  url = url + searchText
  base = 'https://www.youtube.com'
  infoList = []
  retryTime = 4
  while (len(infoList) <= 0 and retryTime > 0):
